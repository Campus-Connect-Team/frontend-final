...
const CommentList = ({ updateComment, deleteComment }) => {
  ...
  const fetchComments = async () => {
    ...
    try {
      const response = await axios.get(`http://15.165.91.27:8080/boards/${boardId}/comments`, {
        headers: {
          Authorization: token
        }
      });
  ...
  const addComment = async (text) => {
    ...
    try {
      await axios.post(
        `http://15.165.91.27:8080/boards/${boardId}/comments`,
        { commentContent: text },
        {
          headers: {
            Authorization: token
  ...
  const handleUpdate = async (id) => {
    ...
    try {
      await axios.patch(
        `http://15.165.91.27:8080/boards/${boardId}/comments/${id}`,
  ...
  const handleDelete = async (id) => {
    ...
    try {
      if (window.confirm('선택한 댓글을 삭제하시겠습니까?')) {
        await axios.delete(`http://15.165.91.27:8080/boards/${boardId}/comments/${id}`, {
          headers: {
            Authorization: token
          }
        });
