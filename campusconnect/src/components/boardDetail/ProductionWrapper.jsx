  ...
  useEffect(() => {
    const fetchBoardData = async () => {
      try {
        ...
        if (boardId) {
          const response = await axios.get(`http://15.165.91.27:8080/boards/${boardId}`, config);
...
const toggleHeart = async () => {
  const accessToken = localStorage.getItem('accessToken');
  const url = `http://15.165.91.27:8080/boards/${boardId}/favorite/${isHeartActive ? 'cancel' : 'register'}`;
  try {
    ...
    const response = await axios.get(`http://15.165.91.27:8080/boards/${boardId}`, {
      headers: {
        Authorization: accessToken
      }
    });
